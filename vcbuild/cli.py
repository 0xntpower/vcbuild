...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--standard",
                        choices=["c11", "c17", "c++17", "c++20", "c++23"],
                        help="C/C++ standard")
...
def cmd_build(config: cfg.Config, profile: str,
    ...
    proj = config.data.get("project", {})
    name = proj.get("name", "unknown")
    arch = proj.get("architecture", "x64")
    proj_type = proj.get("type", "exe")

    output.build_header(name, profile, arch, proj_type)
    ...
    if len(result.warnings) > 10:
        output.detail(f"  ... and {len(result.warnings) - 10} more warning(s)")
    ...
    if result.success and result.output_path:
        out_path = str(result.output_path.relative_to(config.project_root))

    output.build_summary(
        success=result.success,
        duration=result.duration,
        files=result.files_compiled,
